...
fn easy_ticket(title: String, description: String, status: Status) -> Ticket {
    match Ticket::new(title.clone(), description.clone(), status.clone()) {
        Ok(ticket) => ticket,
        Err(TicketNewError::TitleIsEmpty(err)) | Err(TicketNewError::TitleTooLong(err)) => {
            panic!("{}", err);
        }
        Err(TicketNewError::DescriptionIsEmpty) | Err(TicketNewError::DescriptionTooLong(_)) => {
...
impl Ticket {
    pub fn new(
        ...
        if title.is_empty() {
            return Err(TicketNewError::TitleIsEmpty(
        ...
        if description.is_empty() {
            return Err(TicketNewError::DescriptionIsEmpty);
        }
