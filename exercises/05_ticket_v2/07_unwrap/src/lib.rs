...
fn easy_ticket(title: String, description: String, status: Status) -> Ticket {
    match Ticket::new(title.clone(), description.clone(), status.clone()) {
        Ok(ticket) => ticket,
        Err(err) => {
            if err == "Title cannot be empty" || err == "Title cannot be longer than 50 characters"
            {
                panic!("{}", err);
            } else if err == "Description cannot be empty"
                || err == "Description cannot be longer than 500 characters"
            {
