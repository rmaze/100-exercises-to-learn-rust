...
impl<'a> Order<'a> {
    pub fn new(product_name: String, quantity: &'a u32, unit_price: &'a u32) -> Order<'a> {
    ...
    pub fn set_quantity(&mut self, quantity: &'a u32) {
        if *quantity == 0 {
            panic!("Quantity must be greater than zero");
        }
    ...
    pub fn set_unit_price(&mut self, unit_price: &'a u32) {
        if *unit_price == 0 {
            panic!("Unit price must be greater than zero");
        }
